  ...
  <div class="bg-white p-8 rounded-xl shadow-base w-full max-w-3xl space-y-6">
    <div class="grid grid-cols-2 gap-6">
      ...
      <div>
        ...
        <input
          v-model="form.employeeIdentificationNumber"
          type="text"
          placeholder="사번"
          class="w-full border rounded-md p-2 text-sm"
          required
    ...
    <!-- 에러 메시지 -->
    <p v-if="firstErrorMessage" class="text-sm text-red-500">
      {{ firstErrorMessage }}
    </p>
...
onMounted(async () => {
  ...
  if (posResult.status === "fulfilled") {
    positionOptions.value = posResult.value.data.data.map(
      (p) => p.positionName,
    );
  } else {
  ...
  if (deptResult.status === "fulfilled") {
    departmentOptions.value = deptResult.value.data.data.map(
      (d) => d.departmentName,
    );
  } else {
