  ...
  <div class="max-w-4xl mx-auto py-10 px-4 space-y-8">
    ...
    <div class="bg-white p-8 space-y-6">
      <div class="grid grid-cols-2 gap-6">
        <!-- 이름 -->
        <div>
          <label class="text-sm text-gray-600 block mb-3">이름<span class="text-red-500 ml-1">*</span></label>
          <input v-model="form.employeeName" type="text" class="w-full border rounded-md p-2 text-sm" />
          <p v-if="errors.employeeName" class="text-sm text-red-500 mt-1">{{ errors.employeeName }}</p>
        </div>

        <!-- 사번 -->
        <div>
          <label class="text-sm text-gray-600 block mb-3">사번</label>
          <input :value="employeeId" disabled class="w-full border rounded-md p-2 text-sm bg-gray-100 text-gray-500" />
        </div>

        <!-- 전화번호 -->
        <div>
          <label class="text-sm text-gray-600 block mb-3">전화번호<span class="text-red-500 ml-1">*</span></label>
          <input v-model="form.phoneNumber" type="text" class="w-full border rounded-md p-2 text-sm" />
          <p v-if="errors.phoneNumber" class="text-sm text-red-500 mt-1">{{ errors.phoneNumber }}</p>
        </div>

        <!-- 이메일 -->
        <div>
          <label class="text-sm text-gray-600 block mb-3">이메일<span class="text-red-500 ml-1">*</span></label>
          <input v-model="form.email" type="email" class="w-full border rounded-md p-2 text-sm" />
          <p v-if="errors.email" class="text-sm text-red-500 mt-1">{{ errors.email }}</p>
        </div>

        <!-- 입사일 -->
        <div>
          <label class="text-sm text-gray-600 block mb-3">입사일<span class="text-red-500 ml-1">*</span></label>
          <input v-model="form.joinedAt" type="date" class="w-full border rounded-md p-2 text-sm" />
          <p v-if="errors.joinedAt" class="text-sm text-red-500 mt-1">{{ errors.joinedAt }}</p>
        </div>

        <!-- 생년월일 -->
        <div>
          <label class="text-sm text-gray-600 block mb-3">생년월일<span class="text-red-500 ml-1">*</span></label>
          <input v-model="form.birthday" type="date" class="w-full border rounded-md p-2 text-sm" />
          <p v-if="errors.birthday" class="text-sm text-red-500 mt-1">{{ errors.birthday }}</p>
        </div>

        <!-- 연차 -->
        <div>
          <label class="text-sm text-gray-600 block mb-3">연차<span class="text-red-500 ml-1">*</span></label>
          <input v-model.number="form.careerYears" type="number" class="w-full border rounded-md p-2 text-sm" />
          <p v-if="errors.careerYears" class="text-sm text-red-500 mt-1">{{ errors.careerYears }}</p>
        </div>

        <!-- 연봉 -->
        <div>
          <label class="text-sm text-gray-600 block mb-3">연봉</label>
          <input v-model.number="form.salary" type="number" class="w-full border rounded-md p-2 text-sm" />
        </div>
      ...
      <div>
        ...
        <span class="ml-2 text-sm text-gray-500">{{ form.profileImageUrl ? '이미지 선택됨' : '선택된 파일 없음' }}</span>
        <img v-if="form.profileImageUrl" :src="form.profileImageUrl" alt="미리보기" class="mt-2 w-24 h-24 object-cover rounded-full border" />
      </div>
      ...
      <div>
        ...
        <button type="button" @click="openModal" class="border px-3 py-1 rounded-md text-sm text-gray-600 hover:bg-gray-50">
          + 기술 추가
        </button>
      </div>
      ...
      <div class="flex justify-end gap-2">
        <button type="button" @click="isConfirmOpen = true" class="px-6 py-2 bg-primary text-white rounded-md">수정</button>
        <button type="button" @click="cancel" class="px-6 py-2 border border-gray-300 text-gray-700 rounded-md hover:bg-gray-100">취소</button>
      </div>
    </div>
...
function validate() {
  ...
  else if (!/^010\d{8}$/.test(form.phoneNumber)) errs.phoneNumber = '전화번호는 010으로 시작하는 11자리 숫자 형식이어야 합니다.';
  if (!form.email) errs.email = '이메일은 필수 입력 사항입니다.';
  else if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(form.email)) errs.email = '이메일 형식이 올바르지 않습니다.';
  if (!form.birthday) errs.birthday = '생년월일은 필수 입력 사항입니다.';
  if (!form.joinedAt) errs.joinedAt = '입사일은 필수 입력 사항입니다.';
  if (form.careerYears === '' || form.careerYears === null) errs.careerYears = '연차는 필수 입력 사항입니다.';
